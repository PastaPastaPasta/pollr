...
/**
 * SDK helper utilities for working with EvoSDK 4.x.
 *
 ...
 *
 * The SDK returns `Map<Identifier, Document>` from queries. These helpers normalize that
...
export function identifierToBase58(value: unknown): string | null {
  ...
  if (typeof value === 'string') {
    ...
    // Try as base64 (SDK byte-array fields can surface in this form).
 ...
 *
 * `Document#toObject()` returns every Platform `integer` as a bigint, but Pollr treats these as
...
/**
 * Convert a WASM Document instance to the JSON-like shape Pollr expects from queries.
...
export function base58ArrayToBytes(values: string[]): Uint8Array[] {
  ...
  for (const v of values) {
    ...
    // Try base64 (SDK identifiers can surface in this form).
...
export function normalizeSDKResponse(response: unknown): Record<string, unknown>[] {
  ...
  // Handle the SDK's primary Map response format.
 ...
 *
 * Pollr uses base64 for these raw-query comparisons because `sdk.documents.query(...)` receives
